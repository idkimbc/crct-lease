...
const WelcomePage = () => {
	...
	const handleWalletConnect = (chainId) => {
				...
				<CardContent className="space-y-6">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
							{chains.map((chain) => (
								<Button
									...
									onClick={() => handleWalletConnect(chain.id)}
								>
					...
					<div className="mt-6 p-4 rounded-lg bg-slate-50">
						<div className="flex items-center space-x-2">
							<div
								className={`h-3 w-3 rounded-full ${walletConnected ? "bg-green-500" : "bg-slate-300"}`}
							/>
							<span className="text-sm">
								{walletConnected
									? `Connected to ${chains.find((c) => c.id === selectedChain)?.name}`
